...
import com.fastcampus.projectboard2.config.TestSecurityConfig;
...
class ArticleControllerTest {
    ...
    @WithMockUser   // 유저 정보를 모킹해서 넣어준다, 실제 사용자 정보가 이용이 아닌 테스트를 위해 그냥 통과만 시켜준다.
    ...
    void givenNothing_whenRequestingArticlePage_thenRedirectsToLoginPage() throws Exception {
        ...
        mvc.perform(get("/articles/" + articleId))
                ...
                .andExpect(redirectedUrlPattern("**/login"));   // **/는 아무거나 와도 된다는 의미
    ...
    @WithUserDetails(value = "wofudTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)   // 실제 내가 구현한 UserDetails를 사용할 수 있게 해준다, 세션을 시작하기 전에 세팅을 맞춰놔라
    ...
    @WithUserDetails(value = "wofudTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    ...
    void givenArticleIdToDelete_whenRequesting_thenDeletesArticle() throws Exception {
        //given
        Long articleId = 1L;
        String userId = "wofudTesdt";
        willDoNothing().given(articleService).deleteArticle(articleId, userId); // 여기도 없어도 테스트 성공
